...
hook.Add("Think", "TTTCThinkCharge", function()
    ...
    for i = 1, #plys do
        ...
        if not ply:IsActive() or not ply:HasClass() then
            continue
        end

        local classData = ply:GetClassData()

        if not classData or not isfunction(classData.OnThink) then
            continue
        end
...
if SERVER then
    hook.Add("TTTBeginRound", "TTTCSelectClasses", function()
        ...
        if GetGlobalInt("ttt_classes_random") < math.random(100) then
            return
        end

        CLASS.generateClassPool()

        if #CLASS.AVAILABLECLASSES == 0 then
            return
        end
    ...
    hook.Add("PlayerSpawn", "TTTCRemoveClassOnSpawn", function(ply)
        if GetRoundState() == ROUND_ACTIVE then
            ...
            if
                not GetGlobalBool("ttt_classes_keep_on_respawn")
                or classData and classData.surpressKeepOnRespawn
            then
                ply:UpdateClass(nil)
            else
    ...
    hook.Add("PlayerCanPickupWeapon", "TTTCPickupWeapon", function(ply, wep)
        ...
        elseif
            ply:HasClass()
            and ply:HasClassActive()
            and not ply:GetClassData().avoidWeaponReset
        then
            return false
        end
    end)
...
hook.Add("TTTPlayerSpeedModifier", "ClassChargingModifySpeed", function(ply, _, _, refTbl)
    if not IsValid(ply) or not ply.charging then
        return
    end
...
if SERVER then
    local function ClassDeinit(ply)
        if not IsValid(ply) or not ply:HasClassActive() then
            return
        end
...
net.Receive("TTTCActivateClass", function(len, ply)
    ...
    if
        ...
        or not ply:IsActive()
        or ply:GetClassCooldownTS() and ply:GetClassCooldownTS() + ply:GetClassCooldown() > CurTime()
        or hook.Run("TTTCPreventClassActivation", ply)
    then
        reset = true
    end
