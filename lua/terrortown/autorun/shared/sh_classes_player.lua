...
function plymeta:SetClass(class)
    ...
    if class ~= old then
        ...
        if
            classData
            and self.prepareActivation
            and isfunction(classData.OnFinishPrepareAbilityActivation)
        then
    ...
    if
        ...
        and GetGlobalBool("ttt_classes_show_popup", false)
    then
        local classData = self:GetClassData()
        ...
        if classData.lang then
            EPOP:AddMessage(
                ...
                classData.lang.desc
                        and LANG.TryTranslation("tttc_class_" .. classData.name .. "_desc")
                    or nil,
                12
            )
        else
            ...
            EPOP:AddMessage({
                ...
                color = classData.color,
            }, LANG.TryTranslation("ttt2_tttc_class_desc_not_provided"), 12)
        end
    end
end
...
function plymeta:HasClassCooldown()
    ...
    if CurTime() >= timeStart + cooldown then
        return false
    end
...
function plymeta:ClassActivate()
    ...
    if classData.time ~= 0 then
        ...
        if SERVER then
            -- cache the bought item names so they can't removed
...
if SERVER then
    ...
    function plymeta:RemoveAbility()
        ...
        if self.classItems then
            for i = 1, #self.classItems do
                ...
                if
                    self.savedClassInventoryItems
                    and table.HasValue(self.savedClassInventoryItems, classItem)
                then
                    continue
                end
    ...
    function plymeta:SyncClassToTeam()
        ...
        for _, v in ipairs(player.GetAll()) do
            if
                not self:IsInTeam(v)
                or self == v
                or self:GetTeam() == TEAM_INNOCENT
                or self:GetSubRoleData().unknownTeam
                or v:GetSubRoleData().unknownTeam
            then
                continue
            end
    ...
    net.Receive("TTTCSendClassOptions", function()
        ...
        if not IsValid(client) or opt1 == 0 or opt2 == 0 then
            return
        end
