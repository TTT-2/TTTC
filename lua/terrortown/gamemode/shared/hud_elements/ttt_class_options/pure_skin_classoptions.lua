...
if CLIENT then
    ...
    function HUDELEMENT:GetDefaults()
        const_defaults["basepos"] =
            { x = ScrW() - self.optionWidth - self.optionMargin, y = self.optionMargin + 80 }
    ...
    function HUDELEMENT:DrawClassOption(ty, key, name, color, key_width)
        ...
        draw.AdvancedText(
            key,
            "ClassDescOptions",
            x + (self.pad + key_width) * 0.5,
            ty + self.optionHeight * 0.5,
        ...
        draw.RoundedBoxEx(
            0,
            x + key_width + self.pad + 1,
            ty + self.linePad,
            1,
            self.optionHeight - 2 * self.linePad,
            util.GetDefaultColor(color)
        )
        ...
        draw.AdvancedText(
            name,
            "ClassDesc",
            x + self.pad + key_width + line + (w - self.pad - line - key_width) * 0.5,
            ty + self.optionHeight * 0.5,
            util.GetDefaultColor(color),
            TEXT_ALIGN_CENTER,
            TEXT_ALIGN_CENTER,
    ...
    function HUDELEMENT:Draw()
        ...
        self:DrawClassOption(
            y_temp,
            key1,
            CLASS.GetClassTranslation(classData1),
            classData1.color,
            key_width
        )
        ...
        self:DrawClassOption(
            y_temp,
            key2,
            CLASS.GetClassTranslation(classData2),
            classData2.color,
            key_width
        )
    end
    ...
    function HUDELEMENT:ShouldDraw()
        ...
        return HUDEditor.IsEditing
            or (
                client.classOpt1
                and client.classOpt2
                and client:IsActive()
                and GetGlobalBool("ttt2_classes")
                and GetGlobalBool("ttt_classes_option")
            )
    end
end
